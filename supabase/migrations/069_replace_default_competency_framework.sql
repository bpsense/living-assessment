-- 069_replace_default_competency_framework.sql
-- Replaces every school's competency framework with the global Boundless
-- Developmental Skill Baseline (8 domains × 5 age subdomains × age-banded
-- competencies). Old frameworks and all linked rows (assignment_competencies,
-- skill_competencies, competency_dimension_mappings, student_competency_scores)
-- cascade-delete by design — confirmed clean-slate replacement.

-- ============================================================
-- 1. Wipe existing frameworks (cascades through domains, subdomains,
--    competencies, and every linked junction table).
-- ============================================================

delete from competency_frameworks;

-- ============================================================
-- 2. Seed the Boundless baseline per school.
-- ============================================================
...
declare
  ...
  subdomain_id uuid;
begin
  for school_row in select id from schools loop
    -- Framework
    insert into competency_frameworks (school_id, name, description, version, is_default)
    values (school_row.id, 'Boundless Developmental Skill Baseline', 'Default developmental standards for learners ages 4-14, organized by domain and age band. Sources: CCSS, NGSS, CDC, CASEL, EYFS, ISTE.', '1.1', true)
